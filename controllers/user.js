...
const register = ('/register', async (req, res) => {
    ...
    try {
        ...
        res.json({ userCard, contactAdmin: true });
    } catch (err) {
        try {
        ...
        res.json({ errMsg: 'user cant be registered', error: err });
    }
...
const protected = async(req, res) => { 
    try {
        const userCard = await mUserCard.findOne({user: req.user.id})
        res.json({passed: true, userCard})
    } catch (error) {
